...
if [[ -n "$DIR" ]]; then
    ...
    wget https://github.com/cb-linux/kernel/releases/latest/download/bzImage.alt -N -q --show-progress
		ln -s bzImage.alt bzImage
    wget https://github.com/cb-linux/kernel/releases/latest/download/modules.alt.tar.xz -N -q --show-progress
